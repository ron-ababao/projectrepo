...
def filter_log_events(group_name, filter_pat, start=None,
                     stop=None, region_name=None):
    cwlogs = boto3.client('logs', region_name=region_name)
...
start_ts = int(datetime(2022, 7, 19, 5, 22, 
                        tzinfo=timezone.utc).timestamp() * 1000)
                        
end_ts = int(datetime(2022, 7, 19, 5, 24, 
                        tzinfo=timezone.utc).timestamp() * 1000)
...
if __name__=='__main__':
    ...
    for logging in listing_log:
        print(logging)
    
    '''
    listing_stream=list_log_group_streams('/aws/lambda/logtest-ron', 
                    region_name='us-east-1')
    log.info(f"{listing_stream}")
    
    filtering_events=filter_log_events('/aws/lambda/logtest-ron', 'START',
                     start=start_ts, stop=end_ts, region_name='us-east-1')
